...
/**
 * Draws the drawer on the pipeline graph.
 */
export class PipelineGraphDrawer {
  ...
  private gtooltip = new Konva.Label({opacity: 0.75, visible: false});
  ...
  public drawStage() {
    ...
    this.layerLines.on('click', (event) => {
      if (this.ctxMenuShown) {
        const [x, y] = [
          this.stage.getPointerPosition().x, this.stage.getPointerPosition().y
        ];
        let inZone = false;
        this.ctxMenuZone.x.forEach((xRange, i) => {
          const yRange = this.ctxMenuZone.y[i];
          if (x >= xRange[0] && x <= xRange[1] && y >= yRange[0] &&
              y <= yRange[1]) {
            inZone = true;
          }
        });
  ...
  drawJobBox(node, hovered = false) {
    const group = new Konva.Group(
        {x: node.x_offset, y: node.y_offset, id: `jobBox${node.job.id}`});
    ...
    const jobText = this.fittingString(
        this.layerJobBoxes.getContext(), node.job.name,
        this.pgraph.boxWidth - 120);
    ...
    const dotsArea = new Konva.Rect(
        {x: 0, y: 0, width: 23, height: 32, fill: 'transparent'});
    ...
    this.ctxMenuZone.y.push([node.y_offset + 4, node.y_offset + 4 + 32]);
    ...
    dotsBtnGroup.on('click', () => {
      if (this.ctxMenuShown === null ||
          this.ctxMenuShown.job_id !== node.job.id) {
        this.ctxMenuShown = {
          job_id: node.job.id,
          position: box.getAbsolutePosition(),
          size: {width: this.pgraph.boxWidth, height: this.pgraph.boxHeight}
        };
      } else {
        this.ctxMenuShown = null;
      }
  ...
  drawLines() {
    ...
    const bg = new Konva.Rect(
        {x: 0, y: 0, width: this.pgraph.stWidth, height: this.pgraph.stHeight});
  ...
  drawContextMenu() {
    ...
    const menuItems = [
      ...
      {action: 'start', text: 'Start', data: 'M8,5.14V19.14L19,12.14L8,5.14Z'}
    ];

    menuItems.forEach((menuItem, i) => {
      const itemGroup =
          new Konva.Group({x: 0, y: i * this.pgraph.ctxMenuItemHeight});
      const item = new Konva.Text({
        x: 23,
        y: 1,
        text: menuItem.text,
        fontFamily: 'Roboto',
        fill: this.context.pipeline.blocked_managing() ? this.blockedTextColor :
                                                         this.textColor,
        fontSize: 15,
        padding: 10
      });
